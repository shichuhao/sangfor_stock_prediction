...
## 2026-07-09 修复evaluate.py报错 + 历史回测结果不再永远冻结

### 修复
- `daily/evaluate.py`: pandas 3.0 下 `dtype=str` 读入的列是严格string dtype,直接赋值
  float/int(如 `actual_close`)会抛 `TypeError: Invalid value ... for dtype 'str'`。
  GitHub Actions 第一次遇到真实待回填预测(2026-07-08深信服涨停,收盘106.56)时触发了这个bug。
  已改为显式 `str()` 转换,本地用这条真实涨停数据验证通过,并手动触发一次GitHub Actions确认
  CI环境里也跑通。

### 改进:历史回测结果不再永远冻结在同一天
- 问题:`backtest/run_backtest.py` 只在项目上线时手动跑过一次(耗时约11.5分钟,363个交易日),
  没有被排进每日的 GitHub Actions 流程,导致报告里"历史回测"这张表永远停在2025-01-02~2026-07-06,
  不会随时间推移纳入新数据。
- 修复方案(不需要每天重新花11分钟跑完整回测):
  1. `reports/generate_report.py` 新增"累计评估结果"栏目(现为报告第1节),把静态的
     `backtest_results.csv` 和逐日增长的 `predictions.csv`(已回填部分)拼接成一条连续时间线,
     每天运行一次日常流程就能自动反映最新数据,不产生额外计算成本。
  2. 新增 `.github/workflows/monthly_backtest.yml`,每月1号自动用最新全部历史重新训练、
     重跑一遍完整的walk-forward回测,刷新报告第2节的静态基准表本身。
  3. 原"历史回测结果"章节改为报告第2节,明确标注为"静态基准,按月刷新",避免误认为是实时数据。
